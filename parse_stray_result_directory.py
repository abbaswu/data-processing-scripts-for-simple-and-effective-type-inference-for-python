...
parameter_or_return_value_type_inference_results: "[" parameter_or_return_value_type_inference_result ("," parameter_or_return_value_type_inference_result)* "]"
parameter_or_return_value_type_inference_result: "'" type_annotation "'"
type_annotation: recursive_type_annotation
recursive_type_annotation: class_name ("[" type_annotation_list? "]")?
class_name: NAME ("." NAME)*
type_annotation_list: recursive_type_annotation ("," recursive_type_annotation)*
...
def extract_type_annotation_string(type_annotation_tree: Tree) -> str:
    return extract_recursive_type_annotation_string(type_annotation_tree.children[0])
...
def parse(
    ...
    module_search_absolute_path: str,
    ...
    for module_name, module_name_level_query_dict in query_dict.items():
        # python_file_absolute_path: '/tmp/module_search_path/src/meerkat/entrypoints/rest/post/registry.py'
        ...
        result_file_name: str = f"{python_file_absolute_path.replace('/', '-')}_orderred_"
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--module-search-absolute-path', type=str, required=True, help='Module search absolute path, e.g. /tmp/module_search_path')
    parser.add_argument('-s', '--stray-result-directory', type=str, required=True, help='Stray result directory, e.g. Stray/result')
    ...
    result_dict = parse(query_dict, args.module_search_absolute_path, args.stray_result_directory)
